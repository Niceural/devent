...
describe("devent integration tests", () => {
  ...
  describe("createOnChainEvent()", () => {
    const metadataUrl =
      "https://raw.githubusercontent.com/Niceural/devent/main/assets/event1.json";
    const registrationLimit = new anchor.BN(190);
    const minLamportsPrice = new anchor.BN("3450000000");

    it("EventAccount is valid", async () => {
      // arrange
  ...
  describe("registerAttendeeOnChain()", () => {
    it("RegistrationAccount is valid", async () => {
      // arrange
      const [eventPda] = await anchor.web3.PublicKey.findProgramAddress(
        anchor.utils.bytes.utf8.encode("event"),
        new BN()
